...
import { CircleDashed, Trash2, Plus, TestTube2, FileText, CheckCircle, ChevronRight, ChevronLeft, Download, RefreshCw, Layout, Palette } from "lucide-react";
...
interface ResumeInputData {
  ...
  templateStyle: 'modern' | 'classic' | 'minimalist';
}
...
const initialUserInfo: ResumeInputData = {
  ...
  summary: "",
  targetJobRole: "",
  templateStyle: "modern", // Default template
  experience: [
    {
      jobTitle: "",
      company: "",
...
const ResumeBuilder = () => {
  ...
  const handleTemplateChange = (style: 'modern' | 'classic' | 'minimalist') => {
    setUserInfo(prev => ({ ...prev, templateStyle: style }));
  };
  ...
  const handleNextTab = () => {
    ...
    else if (activeTab === "personal") setActiveTab("summary");
  ...
  const handlePrevTab = () => {
    ...
    else if (activeTab === "personal") setActiveTab("templates");
  };
              ...
              <CardContent className="p-0">
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                  <TabsList className="w-full rounded-none border-b bg-gray-50 px-1 sm:px-2 py-0 h-auto flex flex-wrap overflow-x-auto">
                    <TabsTrigger
                      value="templates"
                      className={`py-2 sm:py-3 px-2 sm:px-4 text-xs sm:text-sm rounded-none border-b-2 ${activeTab === "templates" ? "border-blue-500" : "border-transparent"}`}
                    >
                      <Layout className="w-4 h-4 mr-1 sm:mr-2 inline" /> Templates
                    </TabsTrigger>
                    <TabsTrigger
                      value="personal"
                  ...
                  <TabsContent value="templates" className="p-6 space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {/* Modern Template */}
                      <div
                        className={`cursor-pointer border-2 rounded-lg p-4 transition-all ${userInfo.templateStyle === 'modern' ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-gray-200 hover:border-blue-200'}`}
                        onClick={() => handleTemplateChange('modern')}
                      >
                        <div className="h-40 bg-white rounded border border-gray-200 mb-3 flex flex-col p-2 overflow-hidden relative">
                          <div className="h-4 w-1/3 bg-blue-600 mb-2"></div>
                          <div className="h-2 w-1/2 bg-gray-300 mb-4"></div>
                          ...
                          <div className="flex gap-2">
                            <div className="w-1/3 h-20 bg-gray-50"></div>
                            <div className="w-2/3 h-20 bg-gray-50"></div>
                          </div>
                          {userInfo.templateStyle === 'modern' && (
                            <div className="absolute top-2 right-2 bg-blue-500 text-white rounded-full p-1">
                              <CheckCircle className="w-4 h-4" />
                            </div>
                          )}
                        </div>
                        <h3 className="font-bold text-center">Modern</h3>
                        <p className="text-xs text-gray-500 text-center mt-1">Clean, bold headers, sans-serif font. Best for tech & startups.</p>
                      </div>

                      {/* Classic Template */}
                      <div
                        className={`cursor-pointer border-2 rounded-lg p-4 transition-all ${userInfo.templateStyle === 'classic' ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-gray-200 hover:border-blue-200'}`}
                        onClick={() => handleTemplateChange('classic')}
                      >
                        <div className="h-40 bg-white rounded border border-gray-200 mb-3 flex flex-col p-2 overflow-hidden relative font-serif">
                          <div className="h-4 w-full text-center border-b-2 border-black mb-2 flex justify-center items-center font-bold">NAME</div>
                          <div className="h-2 w-full text-center bg-gray-100 mb-4"></div>
                          <div className="h-3 w-full border-b border-gray-300 mb-2"></div>
                          <div className="h-1 w-full bg-gray-200 mb-1"></div>
                          <div className="h-1 w-full bg-gray-200 mb-1"></div>
                          {userInfo.templateStyle === 'classic' && (
                            <div className="absolute top-2 right-2 bg-blue-500 text-white rounded-full p-1">
                              <CheckCircle className="w-4 h-4" />
                            </div>
                          )}
                        </div>
                        <h3 className="font-bold text-center">Classic</h3>
                        <p className="text-xs text-gray-500 text-center mt-1">Traditional, serif font, formal layout. Best for corporate & academic.</p>
                      </div>

                      {/* Minimalist Template */}
                      <div
                        className={`cursor-pointer border-2 rounded-lg p-4 transition-all ${userInfo.templateStyle === 'minimalist' ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-gray-200 hover:border-blue-200'}`}
                        onClick={() => handleTemplateChange('minimalist')}
                      >
                        <div className="h-40 bg-white rounded border border-gray-200 mb-3 flex flex-col p-4 overflow-hidden relative">
                          <div className="h-6 w-1/2 bg-gray-800 mb-4"></div>
                          <div className="h-1 w-full bg-gray-200 mb-2"></div>
                          <div className="h-1 w-full bg-gray-200 mb-2"></div>
                          <div className="h-1 w-2/3 bg-gray-200 mb-6"></div>
                          <div className="h-4 w-1/4 bg-gray-400 mb-2"></div>
                          <div className="h-1 w-full bg-gray-200 mb-1"></div>
                          {userInfo.templateStyle === 'minimalist' && (
                            <div className="absolute top-2 right-2 bg-blue-500 text-white rounded-full p-1">
                              <CheckCircle className="w-4 h-4" />
                            </div>
                          )}
                        </div>
                        <h3 className="font-bold text-center">Minimalist</h3>
                        <p className="text-xs text-gray-500 text-center mt-1">Simple, lots of white space, elegant. Best for design & creative.</p>
                      </div>
                    </div>
                    ...
                    <div className="pt-4 flex justify-end">
                      ...
                      >
                        Next: Personal Info <ChevronRight className="ml-1 h-4 w-4" />
                      </Button>
                    </div>
                  </TabsContent>
